...
class AllDishesRecipie with ChangeNotifier {
  ...
  void getDishesFromStorage() async {
    var box = await Hive.openBox<DishItem>(_boxname);
    _dishesRecipies = box.values.toList();
    notifyListeners();
  }
  ...
  void deleteDish(String id) async {
    var box = await Hive.openBox<DishItem>(_boxname);
  ...
  void addOnce() async {
    ...
    await addDish(_dishesRecipiesToStore[3]);
    await addDish(_dishesRecipiesToStore[4]);
    await addDish(_dishesRecipiesToStore[5]);
  }

  Future<void> addDish(DishItem dishItem) async {
    String id = utils.CreateCryptoRandomString(15);
  ...
  void editDish(DishItem dishItem) async {
    var box = await Hive.openBox<DishItem>(_boxname);
    String id = dishItem.id;
    await box.put(dishItem.id, dishItem);
    int index = _dishesRecipies.indexWhere((element) => element.id == id);
    if (index >= 0) {
      _dishesRecipies[index] = dishItem;
    }
    notifyListeners();
  }
}
